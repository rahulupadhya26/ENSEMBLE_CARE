package com.app.selfcare.calendar
...
import com.app.selfcare.selection.CalendarDetailsLookup
import com.app.selfcare.selection.CalendarKeyProvider
...
class SingleRowCalendar(context: Context, attrs: AttributeSet) : RecyclerView(context, attrs) {
        ...
        this.apply {
            ...
            addOnScrollListener(object : RecyclerView.OnScrollListener() {
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                    ...
                    previousMonthNumber = DateUtil.getMonthNumber(dateList[lastVisibleItem])
                    previousYear = DateUtil.getYear(dateList[lastVisibleItem])
    ...
    fun setDates(newDateList: List<Date>) {
        try {
            ...
            scrollToPosition(0)
            calendarChangesObserver.whenWeekMonthYearChanged(
                DateUtil.getNumberOfWeek(dateList[0]),
                DateUtil.getMonthNumber(dateList[0]),
                DateUtil.getMonthName(dateList[0]),
                DateUtil.getYear(dateList[0]),
                dateList[0]
            )
        } catch (e: Exception) {
            e.printStackTrace()
